...
SELECT
	...
	, 区分コード
	, 勘定科目コード
	, SUM([合計_明細1])/SUM([合計_明細2])/31 AS 応援単価
	, SUM(合計_明細1) AS 合計
	, SUM([合計_明細2]) AS 人員数
FROM 
(
	SELECT 
		*
		, 合計_明細1_本給
			...
			+ 合計_明細1_前月育介金額
			+ 合計_明細1_公的資格手当
			+ 合計_明細1_賃金控除 AS 合計_明細1
		, 合計_明細2_人件費人数会計 AS 合計_明細2
	FROM dat_人事給与データ
) AS A
...
OUEN_RENRAKUHYO_SQL = text("""
 SELECT
	...
	, 延日数
	, 応援単価
	, ROUND(CAST(延日数 AS REAL) / 31, 1) AS 応援人員
	, 応援単価 * 延日数 AS 応援金額
FROM dat_応援連絡票 AS T
LEFT JOIN V_人事給与データ AS J
...
ON T.地区コード = UKEIRE.受け入れ_地区
AND T.課コード = UKEIRE.受け入れ_課コード
LEFT JOIN mst_課コード AS C
ON T.課コード = C.課コード
""")
...
SELECT
    *
    , ROUND(作業時間 * 4134, 0) AS 振替金額
FROM dat_労務費振替依頼書 AS A
LEFT JOIN mst_課コード AS B
ON A.[担当課] = B.[課コード]
""")
